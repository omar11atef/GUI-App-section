...
def calculate_modulus(event=None):  # Add event parameter to handle Enter key
    ...
    try:
        ...
        result_button.config(bg="red" if m > 0 else "blue")  # Background color for positive m
...
root.config(background="#1ecbe1")

# Add Message at the Top
tk.Label(root, text="Calculate the Mod Positive by used : m = q*n + r", 
