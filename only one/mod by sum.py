...
def calculate_modulus(event=None):  # Add event parameter to handle Enter key
    ...
    try:
        ...
        # Calculate -m mod n
        original_m = m  # Store the original value of m for printing
        while m < 0:
            m += n
            if m == 0:  # Special condition to handle exact multiples
                m += n
                break

        # Update the result button text
        result_button.config(text=f"Value: {m}")
        result_button.config(bg="green")  # Change button color to blue
...
root.config(background="#cc10ef")

# Add Message at the Top
tk.Label(root, text="Calculate the Mod negative by used: m sum n", 
...
result_button = tk.Button(frame, text="Result will be shown here", bg="lightgray", width=20)
