...
def calculate_modulus(event=None):  # Add event parameter to handle Enter key
    ...
    try:
        ...
        # Update result on the button
        result_button.config(text=f"{m} = {q}({n}) + {a}")
...
root.config(background="#44BB8F")

# Add Message at the Top
tk.Label(root, text="Calculate the Mod negative by used: r.n = n - r.p", 
